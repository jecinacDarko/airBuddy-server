...
import { getFlights, addBooking, writeFlights, getBookings } from "./fileService";
...
const bookFlight = (bookDTO: BookingDTO): CreatedBooking => {
    ...
    const updatedFlights = [updateFlight(bookDTO.flightTo, flights, numberOfPassengers)];
    
    if (bookDTO.flightFrom.flight_id) {
        updatedFlights.push(updateFlight(bookDTO.flightFrom, flights, numberOfPassengers));
    }

    const flightIds = updatedFlights.map(flight => flight.flight_id);
    writeFlights([...flights.filter(flight => !flightIds.includes(flight.flight_id)), ...updatedFlights])
    
    const newBooking = new CreatedBooking(
        randomUUID(),
        bookDTO
    );
