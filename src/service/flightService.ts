...
const filterItineraries = (flights: Flight[], date: Date, numberOfPassengers: number): Flight[] => {
    return flights.map(flight => {
        return {
            ...flight,
            itineraries: flight.itineraries.filter(itinerary => areDatesOnSameDay(itinerary.depatureAt, date))
                .filter(itinerary => itinerary.avaliableSeats >= numberOfPassengers)
        }
    });
}

const filterFlights = (flights: Flight[], from: string, to: string): Flight[] => {
...
const updateFlight = (
    ...
    return {
        ...flight,
        itineraries: [...flight.itineraries.filter(itinerary => !doesItineraryMatchDetails(itinerary, details)), newItinerary] 
    }
}

const doesItineraryMatchDetails = (itinerary: Itinerary, details: FlightDetails): boolean => {
